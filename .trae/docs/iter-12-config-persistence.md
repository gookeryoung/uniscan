...
实现配置文件持久化，存储在 `~/.pyfilescan/config.yaml` 中，包括窗口几何、
...
- `src/pyfilescan/config.py`：配置持久化模块
  ...
  - `CONFIG_PATH`：默认路径 `~/.pyfilescan/config.yaml`
...
1. **配置位置**：用户主目录 `~/.pyfilescan/config.yaml`，跨平台、多用户隔离
