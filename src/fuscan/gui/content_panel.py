...
封装文件类型树、忽略目录、忽略扩展名三个 TAB 的全部交互逻辑：
``ExtractorTreeModel`` 勾选管理、忽略项编辑器的加载与节流保存、
...
class ContentTabPanel(QObject):  # pyrefly: ignore [invalid-inheritance]
    """配置页内容 TAB 面板控制器：封装文件类型树与忽略项编辑器。

    职责内聚：

    - 构造 :class:`ExtractorTreeModel` 并绑定到 ``file_types_view`` QTreeView
    - 勾选状态变化时即时保存到 :class:`Config` 并更新计数标签
    - 忽略目录/扩展名编辑器 ``textChanged`` 后节流保存（500ms）
    - :meth:`apply_config` 从配置恢复勾选状态与忽略项内容
      （``blockSignals`` 包裹避免触发保存循环）
    - :meth:`flush_pending_save` 立即保存待写入的忽略项（扫描启动前调用）
    ...
    def __init__(
        ...
        # 忽略项节流保存 timer：textChanged 后 500ms 写入配置文件
        ...
        # 信号槽连接：勾选变化 → 即时保存 + 更新计数；编辑器变化 → 节流保存
        ...
        self._exts_edit.textChanged.connect(self._on_ignore_changed)
    ...
    def _save_ignore_to_config(self) -> None:
        ...
        self._config.ignore_extensions = [
            line.strip() for line in self._exts_edit.toPlainText().splitlines() if line.strip()
        ]
        save_config(self._config)
    ...
    def apply_config(self, config: Config) -> None:
        ...
        # 恢复忽略项编辑器内容（blockSignals 避免 textChanged 触发节流保存循环）
        self._dirs_edit.blockSignals(True)
        self._exts_edit.blockSignals(True)
        self._dirs_edit.setPlainText("\n".join(config.ignore_dirs))
        self._exts_edit.setPlainText("\n".join(config.ignore_extensions))
        self._dirs_edit.blockSignals(False)
        self._exts_edit.blockSignals(False)

    def enabled_extensions(self) -> tuple[str, ...] | None:
        """返回启用的文件扩展名列表（全选时 None，Scanner 走原快速路径）。"""
