...
- **多格式扫描**：PDF、Word（.docx）、Excel（.xlsx）、PowerPoint（.pptx）、
  ODF（.odt/.ods）、WPS、纯文本，以及 ZIP/RAR/7Z 压缩包内文件
...
- **快速取消**：扫描中断立即响应，未启动任务被取消，配合大文件跳过百毫秒内退出
- **大文件跳过**：默认跳过超过 100MB 的文件内容读取，避免卡死；可在设置中调整阈值
...
> **取消加速**：点击"取消扫描"后，fuscan 会立即取消所有未启动的扫描任务，
> 不再等待它们进入扫描队列。已开始读取的少量文件（最多等于并发线程数）会
> 快速完成，配合大文件跳过通常在百毫秒内退出扫描循环。
...
选择 YAML 文件后加入列表。若选择的文件已在列表中（如在外部编辑器
修改后想刷新），会弹出询问对话框确认是否重新加载。
...
规则编辑器底部集成正则验证面板，帮助您编写和调试 `mode: regex` 规则：

- **速查手册**：右侧显示 Python `re` 模块常用语法速查（字符类、量词、锚点、
...
```
正则表达式：  (?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})
测试文本：    date: 2024-07-22
...
[1] 6-16: '2024-07-22'
    捕获组: ('2024', '07', '22')
    命名组: {'year': '2024', 'month': '07', 'day': '22'}
```

> **注意**：验证面板使用与扫描引擎一致的 `re.compile(...).finditer(text)`
> 行为，匹配不重叠（从上次结束位置继续），因此测试结果与实际扫描结果一致。
...
- **缓存**：启用扫描缓存可大幅提升重复扫描速度（推荐开启）
  - 缓存路径：默认在用户目录下，可自定义
- **扫描压缩包**：是否扫描 ZIP/RAR/7Z 内文件
- **压缩包密码**：扫描加密压缩包时使用的密码
- **大文件跳过阈值**：超过此大小的文件不读取内容，避免卡死；
  默认 100MB，0 表示不限制（即扫描全部文件内容）
...
- 调整"大文件跳过阈值"（默认 100MB，超过此大小的文件不读取内容）
...
- fuscan 已实现取消加速：点击"取消扫描"后立即取消未启动任务，
  不再等待它们进入扫描队列
- 仅有少量已开始读取的文件（最多等于并发线程数）会快速完成
- 若仍感觉卡顿，可能是某个 worker 正在读取超大文件，降低"大文件跳过阈值"即可
...
- 对 `mode: regex` 规则，可用规则编辑器底部的"正则验证面板"先测试 pattern 是否符合预期
